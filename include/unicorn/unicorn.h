...
#define UC_HOOK_MEM_RW_INVALID (UC_HOOK_MEM_READ_INVALID + UC_HOOK_MEM_WRITE_INVALID)
...
/*
 Get internal REG pointer for direct access.

 @uc: handle returned by uc_open()
 @type:  REG type, GP or FP

 @return: pointer of REGs, or NULL on invalid type.
*/

UNICORN_EXPORT
void *uc_reg_getptr(uc_engine *uc, int type);
...
UNICORN_EXPORT
void * uc_get_tcgctx(uc_engine *uc);
...
UNICORN_EXPORT
void  uc_set_userdata(uc_engine *uc, void *user_data);

UNICORN_EXPORT
void *uc_get_userdata(uc_engine *uc);

UNICORN_EXPORT
void uc_set_errno(uc_engine *uc, int code);

UNICORN_EXPORT
uint64_t uc_get_invaddr(uc_engine *uc);
